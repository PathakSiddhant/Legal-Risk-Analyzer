...
st.markdown("""
    ...
    .feature-card:hover {
        ...
        border-color: #00C9FF;
        box-shadow: 0 15px 30px rgba(0, 201, 255, 0.15);
...
# --- CALLBACK TO CLOSE CHAT ON DOWNLOAD ---
...
# --- C. ANALYSIS PAGE (FILE UPLOADED) ---
    ...
    if st.session_state.analysis_done:
        ...
        with col2:
            ...
            # KEY FIX: on_click=close_chat_handler closes the chat when downloading!
            st.download_button(
                label="📄 Download Risk Report",
                data=pdf_bytes,
                file_name=f"LexiSafe_Report_{uploaded_file.name}.pdf",
                mime="application/pdf",
                use_container_width=True,
                on_click=close_chat_handler
            )
